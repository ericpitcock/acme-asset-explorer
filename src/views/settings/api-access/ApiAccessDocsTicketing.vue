<template>
  <ep-container>
    <div class="intro">
      <h1>Ticketing API</h1>
      <p>Requests to the Atlas Ticketing API enable developers to create, read,
        update and resolve support tickets on behalf of their organization with
        eSentire.</p>
      <ul>
        <li>Create a new support ticket</li>
        <li>Read the information for an existing ticket</li>
        <li>Update an existing ticket with a comment, or change its details</li>
        <li>Resolve a case</li>
      </ul>
    </div>
    <h2>Searching by State Changes</h2>
    <p>Searching by state does not mean the ticket is currently in that state. For
      example, if you want to search for all cases opened in the last week, the
      search is based on the open_date, but the current state could now be closed.
    </p>
    ...
    <div class="lifecycle-item">
      <ep-badge label="New" />
      <p>A recently submitted ticket request that has yet to be assigned to the
        appropriate eSentire analyst.</p>
    </div>
    <div class="lifecycle-item">
      <ep-badge label="Open" />
      <p>Once assigned, the case will be in an open state for the duration of its
        engagement.</p>
    </div>
    <div class="lifecycle-item">
      <ep-badge label="Resolved" />
      <p>Once a case has concluded, its state will be changed to resolved. If a
        comment is posted back to a resolved ticket, the state of the ticket will
        move back to an Open state, until resolved once again.</p>
    </div>
    <div class="lifecycle-item">
      <ep-badge label="Closed" />
      <p>After sitting in a Resolved state for 30 days, a ticket is permanently
    ...
    <div class="code-clock create-post">
      ...
      "short_description":"Customer Test 1 of eSentire's Atlas Ticketing API",
      "description": "This is the body of a test ticket to validate the
      functionality of the API integration between the customer environment and
      eSentire's ticketing system. Feel free to validate this case within the
      Online Ticketing section located at insight.esentire.com/tickets. Additional
    ...
    <div class="code-block resolve-patch">
      {
      "username": "Firsname Lastname"",
      "comments": "Customer comment to Resolve the case via a PATCH command",
      "state": "resolved"
      }
    </div>
  </ep-container>
